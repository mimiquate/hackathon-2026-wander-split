...
[Issue #17](https://github.com/mimiquate/wonder_split/issues/17) is closed as not planned (2026-09-06, grilled with Florencia). The "Armar el viaje" design was refreshed since this plan was first written, and it now has a real Balance screen (see [who-owes-whom.md](./8-15-who-owes-whom.md)) — but still no separate "Resumen"/summary tab or screen anywhere. The design's own trip-fact data (`tripFactsResumen`: total spend, per-traveler share) renders directly inside the Balance tab instead of a distinct screen.
...
No code ships from this doc. See [who-owes-whom.md](./8-15-who-owes-whom.md) for the actual implementation plan covering this scope.
