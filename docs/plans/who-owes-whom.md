...
**Previously an open gap, now resolved:** this plan used to flag that no plan defined the shared trip-level tab strip it assumed. [#8's plan](./build-the-route.md) now owns building that persistent header/tab bar (Ruta/Grupo/Gastos/Balance) as its own Phase 2, since the refreshed design has one and #8 is the first screen where it turns on — this ticket's Balance tab renders as one tab's content inside that shell rather than building any chrome of its own.
